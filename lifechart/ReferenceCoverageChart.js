...
This chart shows the base coverage due to reads aligned across the whole is_reference.<br/><br/>\
For a is_reference of multiple contigs (chromosomes), the initial view will show total<br/>\
...
the average base coveage over a range of the is_reference genome. Click-and-drag or double-<br/>\
...
currently in view or the coverage-per-contig view for the whole is_reference.<br/><br/>\
...
$(function () {
  ...
  function customizeChart() {
    ...
    $("#RC-overlayLabel").attr( "title",
      ...
      "bias for the is_reference regions in view. Whether there is specific or general strand bias "+
    ...
    $("#RC-filterChromLabel").attr( "title",
      "Use this selector to select a particular chromosome (or contig) of the is_reference to view or to go to an overview " +
      "across the whole is_reference by selecting the 'ALL' value. " +
  ...
  function setUnzoomTitle(zoomin) {
    var txt;
    if( zoomin ) {
      txt = "Zoom in to a special view of coverage over the is_reference as a single sequence with all chromsomes\n"+
      ...
      "or to the initial view for coverage off all chomosomes/contigs in the whole is_reference.";
    }
  ...
  // return view whole is_reference or whole contig view
  function unzoomData() {
    if( baseRangeParam.chrom !== '' ) {
